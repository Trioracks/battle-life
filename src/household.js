...
export function cookMeal(campaign) {
  if (campaign.inventory.ingredients < 2) return { state: campaign, inventory: nextInventory(campaign, {}), message: 'Нужно хотя бы два ингредиента.', completed: false };
  const result = advanceCampaign(campaign, { id: 'cook-meal', label: 'Готовит еду', minutes: 35, effects: { energy: -2, leisure: 1 } });
  const state = { ...result.state, inventory: nextInventory(campaign, { ingredients: campaign.inventory.ingredients - 2, cookedMeals: campaign.inventory.cookedMeals + 2, dirtyDishes: campaign.inventory.dirtyDishes + 1 }) };
  return { state, inventory: state.inventory, message: 'Готово две порции.', completed: true };
}

export function eatMeal(campaign) {
  if (campaign.inventory.cookedMeals < 1) return { state: campaign, inventory: nextInventory(campaign, {}), message: 'В холодильнике нет готовой еды.', completed: false };
  const result = advanceCampaign(campaign, { id: 'eat-meal', label: 'Ест дома', minutes: 15, effects: { hunger: 26, leisure: 2 } });
  const state = { ...result.state, inventory: nextInventory(campaign, { cookedMeals: campaign.inventory.cookedMeals - 1, dirtyDishes: campaign.inventory.dirtyDishes + 1 }) };
  return { state, inventory: state.inventory, message: 'Сыт.' , completed: true };
}
...
export function washDishes(campaign) {
  ...
  const state = { ...result.state, inventory: nextInventory(campaign, { dirtyDishes: 0 }) };
  return { state, inventory: state.inventory, message: 'Кухня стала чище.', completed: true };
}
...
export function sleep(campaign, hours) {
  ...
  return { state: result.state, inventory: nextInventory(result.state, {}), message: `Проснулся через ${hours} ч.`, completed: true };
}
